...
const players = [{
  id: '249cf955-8862-423f-a9ac-447d90a5f62e',
  name: undefined,
  email: '123',
  password: '123',
  confirmPassword: '123',
  balance: 1010,
  currency: 'EUR',
  accessToken: 'd0ce13c2-b8c8-4ed4-a135-f63858af877a',
  bets: [  ],
  transactions: [  ]
}
];
...
io.on("connection", (socket) => {
  ...
  socket.on("bet", (data) => {
    ...
    const player = players.find(
        (player) => player.accessToken === data.token
    );

    io.emit("checkBalance", player.balance);
    io.emit("updateBetHistory", player.bets[0]);
  });
